...
function showHelp() {
  ...
  echo "  -u, --user                     (optional) Set odh-manifests repo user to be used for deployment(default opendatahub-io)."
...
images_name=(modelmesh modelmesh-serving modelmesh-runtime-adapter rest-proxy)
...
if [[ -d ${MM_HOME_DIR} ]]; then
  info "Delete exisint ${MM_HOME_DIR} folder"
  rm -rf ${MM_HOME_DIR}
fi
